...
type KeyValueServer interface {
	...
	// All client connections should be closed immediately.
	// Any goroutines running in the background should be signaled to return, but
	// Close() function does not need to wait for all go routines to terminate.
	// Go routines should not deadlock when trying to handle a close signal.
	Close()
}
