...
type Server interface {
	...
	// client are waiting to be returned, or (3) Close() has been called on the
	// server. In the first two cases, the client's connection ID and a non-nil
	...
	// connection with the client has been lost. If Close has been called
	// on the server, it is safe to assume no further calls to Write will be
	// made. In this case, Write must either return a non-nil error, or never
	// return anything.
	...
	// CloseConn, and Close will be made. In this case, Close must either return a
	// non-nil error, or never return anything.
	Close() error
}
